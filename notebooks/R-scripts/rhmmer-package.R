#' This is a modified script from the rhmmer package, which is available on github
...
table %<>%
    dplyr::mutate(description = lines[!grepl("^#", lines, perl = TRUE)] %>%
        sub(
            pattern =
                sprintf("%s *(.*)", paste0(rep("\\S+", N), collapse = " +")),
            replacement = "\\1",
            perl = TRUE
        ))
